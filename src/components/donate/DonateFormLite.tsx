...
const DonateFormLite = () => {
  ...
  React.useEffect(() => {
    if (!data) return;
    enqueueSnackbar("Поздравляем с приобретением Lite подписки!", {
      variant: "success",
    });
  ...
  return (
    ...
    >
      <DialogTitle>Подписка Lite</DialogTitle>
      ...
      <DialogActions sx={{ paddingLeft: "24px", paddingRight: "24px" }}>
        <Button
          ...
          endIcon={
            ...
            >
              {donateItems.filter((item) => item.type === lite[time])[0].cost}
